...
export default function PasteBulkForm({ sessionId, platforms, allowedPlatforms, platformsRestricted, units, requiresUnit }) {
  ...
  const parsed = useMemo(() => {
    ...
    for (const line of lines) {
      ...
      // deteksi lawan SEMUA platform (biar ketauan kalau ternyata platform lain, bukan cuma yang diizinin)
  ...
  return (
    <form action={formAction}>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/3 mb-5">
        ...
        <textarea
          ...
          className="w-full rounded-lg border border-gray-300 dark:border-gray-600 px-3 py-2 text-xs font-mono bg-white dark:bg-gray-800 text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-brand-500 resize-none"
        />
        ...
        <div className={`grid grid-cols-1 gap-4 mt-4 ${requiresUnit ? 'sm:grid-cols-2' : ''}`}>
          <div>
            <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
      ...
      {parsedItems.length > 0 && (
        <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/3 mb-5">
          <div className="px-5 py-3 border-b border-gray-100 dark:border-gray-800 flex flex-wrap items-center justify-between gap-2">
            <span className="text-xs font-medium text-gray-500 dark:text-gray-400">
              {parsedItems.length} URL terdeteksi
            </span>
      ...
      <div className="flex gap-3">
        <button
          type="submit"
          disabled={!canSubmit}
          className="rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-600 disabled:opacity-40"
        >
          {isPending ? 'Menyimpan...' : `Tambahkan ${parsedItems.length > 0 ? `(${parsedItems.length} link)` : ''}`}
        </button>
        <Link
          href={`/sesi-rekap/${sessionId}`}
